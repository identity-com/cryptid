...
const stubResolveDID = async (did: string, key: Keypair, registered: boolean):Promise<DIDDocument> => {
  ...
  document.verificationMethod = [{
    id: did + '#' + alias, controller: did, type: '...'
  }]
...
describe('transactions/did/removeKey', () => {
  ...
  it('should create an update instruction if the DID is registered', async () => {
    ...
    const transaction = await removeKey(connection(), did, key.publicKey, alias, [normalizeSigner(key)]);
  ...
  it('should retain all other data', async () => {
    ...
    await removeKey(connection(), did, key.publicKey, alias, [normalizeSigner(key)]);
  ...
  it('should throw an error if the document verification methods array is missing', async () => {
    ...
    const shouldFail = removeKey(connection(), did, key.publicKey, alias, [normalizeSigner(key)]);
