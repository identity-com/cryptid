...
use cryptid_signer::instruction::cancel_transaction::CancelTransactionBuild;
...
#[tokio::test]
async fn cancel_transaction_test() {
    ...
    let cancel_build = CancelTransactionBuild {
    ...
    program_values
        .send_transaction(
            &[build_instruction!(
                program_values.cryptid_program_id,
                CryptidInstruction,
                CancelTransaction(cancel_build)
            )
            .expect("Could not build cancel")],
            once(&return_account),
            true,
            false,
        )
