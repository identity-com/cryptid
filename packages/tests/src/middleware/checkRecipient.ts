...
describe("Middleware: checkRecipient", () => {
  ...
  const checkRecipient = (transactionAccount: Keypair) =>
    ...
    checkRecipientMiddlewareProgram.methods
      .executeMiddleware()
      .accounts({
        middlewareAccount,
        transactionAccount: transactionAccount.publicKey,
  ...
  before("Set up middleware PDA", async () => {
    [middlewareAccount, middlewareBump] =
      await deriveCheckRecipientMiddlewareAccountAddress(
        authority.publicKey,
        0
      );

    await checkRecipientMiddlewareProgram.methods
      .create(recipient.publicKey, 0, middlewareBump)
      .accounts({
        middlewareAccount,
        authority: authority.publicKey,
      })
