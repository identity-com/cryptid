...
// Creates a reference to an account, that is passed as part of cryptid instruction data for each account.
const fromTransactionAccountMeta = (
  tam: TransactionAccountMeta,
  account: PublicKey
): AccountMeta => ({
  pubkey: account,
...
export const toInstructionData =
  (accounts: PublicKey[]) =>
  (instruction: TransactionInstruction): InstructionData => {
    ...
    const accountMetas = instruction.keys.map(
      ({ pubkey, isSigner, isWritable }) =>
        toTransactionAccountMeta(
          accounts.findIndex((a) => a.equals(pubkey)),
          isWritable,
          isSigner
        )
    );
...
const accountMetaReducer =
  (cryptidAccount: CryptidAccountDetails) =>
  (
    accumulator: { [k: string]: AccountMeta },
    { pubkey, isSigner, isWritable }: AccountMeta
  ): { [k: string]: AccountMeta } => {
    const isCryptidAccountMeta = (accountMeta: AccountMeta) =>
      accountMeta.pubkey.equals(cryptidAccount.address);

    const mergeAccountMetas = (
      existingAccountMeta: AccountMeta | undefined,
      newAccountMeta: AccountMeta
    ): AccountMeta => {
      const mergedAccountMeta = existingAccountMeta
        ? {
            ...
            isWritable:
              existingAccountMeta.isWritable || newAccountMeta.isWritable,
          }
        : newAccountMeta;
    ...
    return {
      ...accumulator,
      [pubkey.toBase58()]: mergeAccountMetas(accumulator[pubkey.toBase58()], {
        pubkey,
        isSigner,
        isWritable,
      }),
...
export const extractAccountMetas = (
  ...
  Object.values(
    instructions
      .flatMap((instruction) => [
        ...instruction.keys,
        toAccountMeta(instruction.programId, false, false),
      ])
...
export const transactionAccountMetasToAccountMetas = (
  transactionAccountMetas: TransactionAccountMeta[],
  accounts: PublicKey[],
  cryptidAccount: CryptidAccountDetails
): AccountMeta[] =>
  Object.values(
    transactionAccountMetas
      .map((tam) => fromTransactionAccountMeta(tam, accounts[tam.key]))
      .reduce(accountMetaReducer(cryptidAccount), {})
  );

export const uniqueKeys = uniqBy<PublicKey, string>((k) => k.toBase58());

export const getCryptidAccountAddress = (
