...
export class CryptidService {
    ...
    static permissionless(connection: Connection, opts: ConfirmOptions = {}): CryptidService {
    ...
    public async loadAccountDetails(did: string, address: PublicKey, cryptidAccount: CryptidAccount): Promise<CryptidAccountDetails> {
        const middlewareAccount = cryptidAccount.middleware &&
            await this.program.provider.connection.getAccountInfo(cryptidAccount.middleware);

        // TODO support chaining
        const middleware: [PublicKey, AccountInfo<Buffer>] | undefined = middlewareAccount && cryptidAccount.middleware ? [
            cryptidAccount.middleware,
            middlewareAccount
        ] : undefined;

        return CryptidAccountDetails.fromAccounts(did, address, cryptidAccount, middleware ? [middleware] : []);
    }

    public async findAllAccounts(did: string, offset = 0, page = 20): Promise<CryptidAccountDetails[]> {
        // By convention, the first account is the generative case
        const defaultAccountDetails = await CryptidAccountDetails.defaultAccount(did)
        ...
        const addresses = await Promise.all(
            range(adaptedOffset, offset + page).map(index =>
                getCryptidAccountAddress(didAccount, index)[0]
            )
        );

        const rawAccounts = await this
            .program
            .account
            .cryptidAccount
        ...
        return [...(offset === 0 ? [defaultAccountDetails] : []), ...accountDetails];
    }

    public listPendingTransactions(account: CryptidAccountDetails):Promise<[PublicKey, TransactionAccount][]> {
    ...
    public async createAccount(details: CryptidAccountDetails): Promise<string> {
        const lastMiddleware = details.middlewares.length > 0 ? details.middlewares[details.middlewares.length - 1] : undefined;
        return this.program.methods.create(
    ...
    public async execute(account: CryptidAccountDetails, transactionAccountAddress: PublicKey, signers: Keypair[] = []): Promise<Transaction> {
        const txAccount = await this.program.account.transactionAccount.fetch(transactionAccountAddress);
        // TODO fix - the TransactionAccount type has `state` as type `never`
        const transactionAccount: TransactionAccount = txAccount as unknown as TransactionAccount;
    ...
    public async directExecute(account: CryptidAccountDetails, transaction: Transaction): Promise<Transaction> {
        const cryptidTransaction = CryptidTransaction.fromSolanaInstructions(
            account,
            this.authorityKey,
            transaction.instructions,
