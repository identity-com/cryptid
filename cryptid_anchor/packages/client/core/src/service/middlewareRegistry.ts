...
type MiddlewareContext = { client: MiddlewareClient<GenericMiddlewareParams>, accounts: Middleware }
export class MiddlewareRegistry {
    ...
    public register<C extends GenericMiddlewareParams>(programId: PublicKey, middleware: MiddlewareClient<C>) {
        this.middleware.set(programId.toBase58(), middleware);
    }

    public getMiddlewareContexts(details: CryptidAccountDetails): MiddlewareContext[] {
        return details
            .middlewares
            .map((middleware) => {
                const middlewareClient = this.middleware.get(middleware.programId.toBase58());
                if (!middlewareClient) {
                    throw new Error(`No middleware registered for program ID ${middleware.programId.toBase58()}`);
                }
