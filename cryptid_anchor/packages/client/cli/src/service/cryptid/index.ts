...
export const getKeys = async (cryptid: Cryptid): Promise<string[]> => {
  ...
  .map((verificationMethod: VerificationMethod) => ({
    alias: verificationMethod.id.slice(Math.max(0, verificationMethod.id.indexOf('#') + 1)),
...
export const getRecipientAddressForDid = async (
  did: string,
  index = 0,
): Promise<PublicKey> => util.getCryptidAccountAddressFromDID(did, index).then(([address]) => address)
