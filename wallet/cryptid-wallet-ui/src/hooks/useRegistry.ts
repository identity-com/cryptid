...
export const useRegistry = (): RegistryContext => {
  ...
  const [registeredEthereumDIDs, setRegisteredEthereumDIDs] = useState<
    string[]
  >([]);
  ...
  const getRegisteredDIDsForEthAddress = useCallback(
    (ethAddress: string): Promise<string[]> =>
      ReadOnlyRegistry.forEthAddress(
        ethAddress,
        connection,
        cluster
      ).listDIDs(),
  ...
  const getRegisteredDIDsForAccount = useCallback(
    (account: PublicKey): Promise<string[]> =>
      ReadOnlyRegistry.for(account, connection, cluster).listDIDs(),
    [connection]
  );
  ...
  const loadDIDs = useCallback(() => {
    ...
    if (wallet.publicKey) {
      ...
      getRegisteredDIDsForAccount(wallet.publicKey).then(
