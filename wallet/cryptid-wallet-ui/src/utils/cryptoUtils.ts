...
export const decrypt = async (
  ...
  did: string,
  wallet: MessageSignerWalletAdapter
): Promise<UnencryptedPayload> => {
  const lexi = new LexiWallet(wallet, did, {
    publicSigningString: payload.seed,
  });

  const decryptedPayload = (await lexi.decrypt(payload.jwe)) as Omit<
    UnencryptedPayload,
    "data"
  > & { data: string };
