...
const getService = (
  ...
  DidSolService.build(new DidSolIdentifier({ authority, clusterType }), {
    wallet: wallet ? toWallet(wallet) : undefined,
    connection,
  });

const getServiceFromDID = (
  did: string,
  wallet?: WalletContextState,
  connection?: Connection
) =>
  DidSolService.build(DidSolIdentifier.parse(did), {
    wallet: wallet ? toWallet(wallet) : undefined,
    connection,
  });

export const keyToDid = (
  key: PublicKey,
...
export const findPFP = (document: DIDDocument): string | undefined =>
  document.service?.find((s) => s.type === "PFP")?.serviceEndpoint;

export const isVerificationMethod = (
  entry: DIDVerificationMethod | ServiceEndpoint
): entry is DIDVerificationMethod => entry.hasOwnProperty("controller");

export const resolveDID = (
  did: string,
  connection: Connection
): Promise<DIDDocument> =>
  getServiceFromDID(did, undefined, connection).resolve();
...
export const addServiceToDID = async (
  did: string,
  wallet: WalletContextState,
  service: Service,
  connection: Connection
...
export const removeServiceFromDID = async (
  did: string,
  wallet: WalletContextState,
  identifier: string,
  connection: Connection
...
export const getVerificationMethodFlags = async (
  did: string,
  wallet: WalletContextState,
  fragment: string,
  connection: Connection
): Promise<VerificationMethodFlags | undefined> => {
  if (!wallet.publicKey) throw new Error("Wallet is not connected");
  ...
  return account.verificationMethods.find((vm) => vm.fragment === fragment)
    ?.flags;
};

export const addVerificationMethodToDID = async (
  did: string,
  wallet: WalletContextState,
  key: AddVerificationMethodParams,
  connection: Connection
): Promise<void> => {
  if (!wallet.publicKey) throw new Error("Wallet is not connected");
  ...
  await didSolService
    .addVerificationMethod(key)
    .withAutomaticAlloc(wallet.publicKey)
    .rpc();
};

export const removeVerificationMethodFromDID = async (
  did: string,
  wallet: WalletContextState,
  identifier: string,
  connection: Connection
...
export const setOwned = async (
  ...
  // get current flags
  const flags =
    (await getVerificationMethodFlags(did, wallet, fragment, connection)) ||
  ...
  didSolService.setVerificationMethodFlags(
    fragment,
    flags.array,
    wallet.publicKey
  );
...
export const isMigratable = async (
  did: string,
  connection: Connection
): Promise<boolean> => {
...
export const getDIDAddress = async (
  did: string,
  connection: Connection
...
export const isInitialized = (
  ...
  getDIDAccount(did, connection).then((account) => !!account);
